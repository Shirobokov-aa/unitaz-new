...
export function CollectionPreviewManager({ initialData }: CollectionPreviewManagerProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updatedPreviews = previews.map((p) => (p.id === selectedPreview.id ? selectedPreview : p));
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {previews.map((preview) => (
            ...
            >
              ...
              <div className="mt-2 flex gap-2">
                <Button variant="outline" onClick={() => setSelectedPreview(preview)}>
                  Редактировать
                </Button>
                <Button asChild variant="secondary">
                  <Link href={`/admin/collections/edit/${preview.id}`}>Детальная информация</Link>
                </Button>
              </div>
            </div>
          ))}
      ...
      {selectedPreview && (
        <form onSubmit={handleSubmit} className="space-y-4 p-4 border rounded sticky top-4">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700">Направление флекса</label>
            <Select
          ...
          <div className="flex gap-2">
            ...
            <Button type="button" variant="destructive" onClick={() => setSelectedPreview(null)}>
              Отмена
            </Button>
          </div>
        </form>
